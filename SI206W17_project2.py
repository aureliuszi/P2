...
# See the PDF of instructions for more detail.
...
def find_urls(astring):
    a = re.findall("https*://[A-z]*[.][A-z][A-z/.0-9]+",astring)
    return a
...
def get_umsi_data():
    try:
        ...
        return dictionary["umsi_directory_data"]
        ...
        html = resp.text
        list.append(str(html))
        while counter <=11:
            resp = requests.get(base_url+"&page="+str(counter), headers={'User-Agent': 'SI_CLASS'})
            html = resp.text
            list.append(str(html))
            counter += 1

        f = open("206project2_caching.json", 'w')
        CACHE_DICTION["umsi_directory_data"]=list
...
for item in htmldoc:
    ...
    for p in people: # for each beautifulsoup object
...
def get_five_tweets(s):
    if s in CACHE_DICTION:  # if it is...
        twitter_results = CACHE_DICTION[s]  # grab the data from the cache!

    else:
        twitter_results = api.search(q=s)  # get it from the internet
        # but also, save in the dictionary to cache it!
        CACHE_DICTION[s] = twitter_results  # add it to the dictionary -- new key-val pair
    ...
    listi = []
    while count < 5:
        for item in twitter_results['statuses']:
